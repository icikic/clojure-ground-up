...
(defn dot-product
  "Finds the inner product of two x, y, z coordinate maps. See http://en.wikipedia.org/wiki/Dot_product"
  [c1 c2]
  (+ (* (:x c1) (:x c2))
     (* (:y c1) (:y c2))
     (* (:z c1) (:z c2))))
...
(defn projection
  "The component of coordinate map a in the direction of coordinate map b. See http://en.wikipedia.org/wiki/Vector_projection."
  [a b]
  (let [b (unit-vector b)]
    (scale (dot-product a b) b)))

(defn rejection "The component of coordinate map a *not* in the direction of coordinate map b."
  [a b]
...
(def earth-equatorial-speed
  "How fast points on the equator move, relative to the center of the earth, in meters/sec."
  (/ (* 2 Math/PI earth-equatorial-radius) earth-day))
...
(defn centaur
  "The upper rocket stage. http://en.wikipedia.org/wiki/Centaur_(rocket_stage) http://www.astronautix.com/stages/cenaurde.htm"
  []
  {:dry-mass 2361
   :fuel-mass 13897
   :isp 4354
...
(defn orientation
  ...
  (<= (first ascent) (:time craft) (last ascent)) (:position craft)
...
(defn fuel-rate
  ...
  (<= (:fuel-mass craft) 0) 0
  ; Ascent burn
  (<= (first ascent) (:time craft) (last ascent)) (:max-fuel-rate craft)
  ; Circularization burn
  (<= (first circularization) (:time craft) (last circularization)) (:max-fuel-rate craft)
...
(defn stage
  ...
  (cond
  ; Still fuel left
  (pos? (:fuel-mass craft)) craft
  ; No remaining stages
  (nil? (:next-stage craft)) craft
  ; Stage!
  :else (merge (:next-stage craft)
...
(defn engine-force
  ...
  ; (println craft)
  ; (println "t "
  ; (:time craft) "alt" (altitude craft) "thrust" (thrust craft))
...
(defn step
  ...
  (let [craft (stage craft)]
    ...
    :position (merge-with + (:position craft) (scale dt (:velocity craft)))
    ; And our velocity changes based on our acceleration
    :velocity (merge-with + (:velocity craft) (scale dt (acceleration craft))))))
...
(defn crash-time
  ...
  [trajectory] (:time (last (flight trajectory))))
...
(defn apoapsis2
  ...
  [trajectory]
  (second
    (drop-while #(<= (first %) (second %)) (partition 2 1
                                                      (map altitude
                                                           (flight trajectory))))))


(fn [val keys] (let [r {}] (map #(assoc r % val) keys)))

(= (apply merge (fn [val keys] (map #(assoc {} % val) keys) 0 [:a :b :c]) {:a 0 :b 0 :c 0}))
(apply merge (fn [val keys] (map #(assoc {} % val) keys) 0 [:a :b :c]) )

((fn [val keys] (let [r {}] (map #(assoc r % val) keys))) 0 [:a :b :c])